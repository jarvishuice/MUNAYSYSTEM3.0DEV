...
@ABONOS.post("/Espacios")
async def RegistraPAgoEspacios(Abono:AbonosEntity):#auten:str=Depends(aut)):
    
    trigger=core.registrarAbonoEspacios(Abono)
    ...
    if trigger['status'] ==True:
       ...
       raise HTTPException(400,trigger['mesagge'])
@ABONOS.get("/Espacios/{idCliente}/{sede}")
async def consultaEspacios(idCliente,sede):
    trigger=core.getAbonoEspacios(idCliente,sede)
