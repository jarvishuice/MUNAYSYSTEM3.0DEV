...
class AbonoDAO(IAbonos,ConectionsPsqlInterface):
    ...
    def registrarABonoEspacios(self,abono:AbonosEntity):
        try:
            abono.id= time.time()
            conection= self.connect()
            if conection['status'] == False:
                ...
                return ResponseInternal.ResponseInternal(False,"error de conexion a la base de datos",None)
            with self.conn.cursor() as cur:
                cur.execute(f"insert into abonos_espacios (id,idcliente,idpago,status,monto,sede)  values('{abono.id}',{abono.idCliente},'{abono.idPago}','{abono.status}',{abono.monto},'{abono.sede}')")
                self.conn.commit()
                return ResponseInternal.responseInternal(True,f"Abono registrado con exito!!! detail [{abono}]",abono)
    ...
    def getAbonoEspacios(self,idCliente:int,sede:str): 
        try:
            data = []
            ...
            if conection['status'] == False:
                ...
                return ResponseInternal.ResponseInternal(False,"error de conexion a la base de datos",None)
            with self.conn.cursor() as cur:
                cur.execute(f"SELECT COALESCE(SUM(monto), 0)  from abonos_espacios a  where idcliente = {idCliente} and sede = '{sede}'")
                for i in cur:
                    data.append(i[0])
                return ResponseInternal.responseInternal(True,f"Abono registrado con exito!!! detail ",data[0])
