...
from core.Implements.pagos.pagosEspaciosDAO import PagosEspaciosDAO,PagosEntity,Logs
class TransferenciasEspaciosWADAO(ConectionsPsqlInterface):
        def __init__(self):
            super().__init__()
        INTEGRACION= PagosEspaciosDAO()    
                    ...
                    if pago['status']==True:
                        ...
                        if conection['status']==True:
                            with self.conn.cursor() as cur:
                                cur.execute(f"""INSERT INTO public.wallet_espacios (idcliente, id, idpago, status, monto) VALUES({idCliente}, '{idPago}', '{idPago}', 'APLICADO', {MONTO});
                                            INSERT INTO public.abonos_espacios (id, idcliente, idpago, status, monto, sede) VALUES('{idPago}', {idCliente}, '{idPago}', 'APLICADO', {monto}, '{sede}');""")
                            self.conn.commit()
