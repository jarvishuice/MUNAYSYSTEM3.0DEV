...
#group by c.nombre ,c.id""")
                    cur.execute(f""" SELECT SUM(o.total) AS deuda, COUNT(o.total) AS cantidad_ordenes,
                                    ...
                                    c.id,
                                    c.ci,
                                    (SELECT COALESCE(SUM(a.monto), 0) FROM abonos_espacios a WHERE a.idcliente = c.id AND a.sede = '{sede}') AS saldo
                                    FROM 
                                    ordenes_espacios o
                                    INNER JOIN 
                    ...
                    if count > 0:
                        for i in cur:
                            data.append(DeudaClienteCoffeShopEntity(idCliente=int(i[3]),ci=str(i[4]),nombre=str(i[2]),cantidadOrdenes=int(i[1]),deuda=float(i[0]),abono=float(i[-1])))     
...
""")
    ...
    def closeDeudaClientBySede(self,pago: PagosEntity, Rwallet: float,Dwallet: float) -> PagosEntity:
        print("validacin  de paso por close deuda cliente by sede espacio")
        try:
            ...
            if conexion['status'] ==True:       
               ...
               if rPago['status']==True:
                   abonoDescuento=self.integracionAbono.registrarABonoEspacios(AbonosEntity(id=str('x'),idCliente=int(pago.idcliente),idPago=str(rPago['response'].id),status= str("aplicado").upper(),monto=float(Dwallet)*-1,sede=str(rPago['response'].sede)))
