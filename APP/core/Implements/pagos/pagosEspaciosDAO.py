...
class PagosEspaciosDAO(ConectionsPsqlInterface,IPagos):
    ...
    def registrarAbono(self,pagoData: PagosEntity) -> PagosEntity:
        try:
            ...
            if pago['status']==True:
                abono=self.abonos.registarAbonoEspacios(AbonosEntity(id=str(time.time()),idCliente=int(pagoData.idcliente),idpago=str(pago['response'].id),status=str('aplicado'),monto=float(pagoData.monto),sede=pagoData.sede))
    ...
    def registroMultipago(self,pagoData:PagosEntity)->PagosEntity:
        try:
            pagoData.motivo=f"Abono Espacios {pagoData.sede}"
            pago=self.registrarPago(pagoData)
            if pago['status']==True:
                abono=self.abonos.registrarABonoEspacios(AbonosEntity(id=str(time.time()),idCliente=int(pagoData.idcliente),idPago=str(pago['response'].id),status=str('aplicado'),monto=float(pagoData.monto),sede=pagoData.sede))
                if abono['status'] ==True:
                    return ResponseInternal.responseInternal(True,"Abono registrado de anera correcta",pago['response'])
                else:
                    return ResponseInternal.responseInternal(False, abono['mesagge'],None)
