...
inner join tazadollar t on precio=precio where idformadepago ={selector} AND p.sede ='{sede}' AND date(p.fechapago) ='{ano}-{mes}-{day}' and t.id = p.idtaza ;
...
WHERE DATE(o.fechapedido) = '{ano}-{mes}-{day}'
    AND o.sede = '{sede}'
    and i.id=idproducto
GROUP BY i.nombre order by i.nombre asc;
                                """);       
                    self.conn.commit()
                    count= cur.rowcount
                    if count > 0 :
