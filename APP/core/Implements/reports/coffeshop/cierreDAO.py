...
inner join tazadollar t on precio=precio where idformadepago ={selector} AND p.sede= '{sede}' AND date(p.fechapago) =' {datetime.date.today()} ' and t.id = p.idtaza ;
...
where c.id=o.idcliente and status= 'por pagar' and sede='{sede}' and date(o.fechapedido)<> '{datetime.date.today()}';
    ...
    def generarCierre(self,sede: str):
        try:
            ...
            datos=ReporteCoffeshopEntity(ventasPorProductos=self.__ventasPorProducto__(sede)['response'],
                                        deudaCliente=self.__DeudaCleinteBysede__(sede)['response'],
                                         ...
                                         detallePagos=detallePago,puntoCount=self.__countPunto__(sede)['response'],
    ...
    def integracion(self,sede:str):
        try:    
           ...
           # html=''
            ...
            return ResponseInternal.responseInternal(False,"integracion del core cierre coffe fallida",datos)           
