...
from core.utils.plantillaHTMLCierreByFEechaESpacios import PlantillaHTMLCierreJornadaByFechaEspacios
import pdfkit
...
class ReportCierreByDIaDAO(ConectionsPsqlInterface,ICierre):
    validacion=validationPedidosData()
    plantilla=PlantillaHTMLCierreJornadaByFechaEspacios()
    ...
    def __init__(self,dia:int,mes:int,year:int):
        ...
        self.mes = mes
        self.year = year
        self.fecha = str(f"{self.dia}-{self.mes}-{self.year}")
        super().__init__()
        print(self.fecha)
...
where c.id=idcliente  and status= 'pagado' and  sede='{sede}' and date(o.fechapedido)= ' {self.fecha} ' 
group  by c.nombre
                                """)
...
inner join clientes c on c.nombre =c.nombre
where c.id=o.idcliente and status= 'por pagar' and sede='{sede}' and date(o.fechapedido)='{self.fecha}'
                                """)
...
and date(p.fechapago) =' {self.fecha} ' and monto > 0 and f.id=p.idformadepago and t.id=p.idtaza     
...
inner join tazadollar t on precio=precio where idformadepago ={selector} AND sede ='{sede}' AND date(p.fechapago) =' {self.fecha} ' and t.id = p.idtaza ;
    ...
    def __ventasPorProducto__(self,sede: str) -> list[VentasPorProducto]:
        data=[]
        try:
            conexion=self.connect()
...
WHERE DATE(o.fechapedido) = '{self.fecha}'
    AND o.sede = '{sede}'
    and i.id=idproducto
GROUP BY i.nombre;
                                """);       
    ...
    def __DetallesPedidos__(self,sede: str) -> list[DetallesPedidos]:
        data=[]
        try:
            conexion=self.connect()
...
where  p3.id = idproducto and c.id=o.idcliente  and o.id=p2.idorden  and date(o.fechapedido) ='{self.fecha}' and o.sede='{sede}';      
...
ORDER BY c.nombre ASC;
            ...
            Logs.WirterTask("Finalizada la ejecucion de registros de productos ")
            self.disconnect()
    def generarCierre(self,sede: str):
        try:
            ventaCLientes=[]
            detallePago=[]
            ...
            html=''
            output_path =f"assets/reports/espacios/cierre/CierreDeJornada{sede}{self.fecha}.pdf"
            print(type(self.__VentasClientes__(sede)['response']))
            
            datos=ReporteEspaciosEntity(ventasPorProductos=self.__ventasPorProducto__(sede)['response'],deudaCliente=self.__DeudaCleinteBysede__(sede)['response'],detallePedidos=self.__DetallesPedidos__(sede)['response'],walletDisponibles=self.__walletDisponible__(sede)['response'],ventasPorCliente=ventaCLientes,ordenesAbiertasHistoricas=self.__OrdenesHistoricas__(sede)['response'],ordenesAbiertas=self.__OrdenesAbiertas__(sede)['response'],detallePagos=detallePago,puntoCount=self.__countPunto__(sede)['response'])
            
                
            print(type(self.__VentasClientes__(sede)['response']))
            html=self.plantilla.getHTML(datos,sede,dia=self.dia,mes=self.mes,year=self.year)
            pdf=pdfkit.from_string(html,output_path,options=self.OPTIONS)
            return ResponseInternal.responseInternal(True,"reporte de cierrede joranan by fecha  espacios generado con exito",output_path)
