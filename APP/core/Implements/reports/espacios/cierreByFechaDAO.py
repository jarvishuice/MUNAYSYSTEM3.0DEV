...
from core.utils.plantillaHtmlCierreEspacios import PlantillaHTMLCierreJornada
...
class ReportCierreDAO(ConectionsPsqlInterface,ICierre):
    validacion=validationPedidosData()
    plantilla=PlantillaHTMLCierreJornada()
    __coreDeudas=DeudaCientesEspaciosDAO()
    OPTIONS = {
                      'page-size': 'Letter', 
      'margin-top': '0.75in',
      'margin-right': '0.75in',
    ...
    def __AccionesUsuarios(self,sede):
        ...
        datos = acciones.getAllActivityByAlmacenToday(sede,"espacios")
    ...
    def __VentasClientes__(self,sede: str,inicio:str,fin:str) -> list[VentasPorClientes]:
        data=[]
        try:
            ...
            if conexion['status'] == True:
              with self.conn.cursor() as cur :
                    ...
                    cur.execute(f"""
                          select sum(total),c.nombre from ordenes_espacios o
...
order by c.nombre asc;
                    ...
                    if count > 0 :
                        for i in cur :
                         #print(f"\t debug VnetasClientes {i}")
            ...
            Logs.WirterTask(f"{self.ERROR} error en la base de datos linea 75 CIerreByfhecaDao detail{e}")
    ...
    def __OrdenesAbiertas__(self,sede: str,inicio:str,fin:str) -> list[OrdenesAbiertas]:
        data=[]
        try:
            conexion=self.connect()
...
where c.id=o.idcliente and status= 'por pagar' and sede='{sede}' and date(o.fechapedido)>='{inicio}' and date(o.fechapedido)<= '{fin}';
            ...
            Logs.WirterTask(f"{self.ERROR} error en la base de datos linea 118 cierreByFechaDAO detail{e}")
            return ResponseInternal.responseInternal(False,"ERROR EN LA BASE DE DATOS",None)
        finally:
            Logs.WirterTask("Finalizada la ejecucion de registros de productos ")
            self.disconnect()
           
    def __DetallesPagos__(self,sede: str,inicio:str,fin:str) -> list[DetallePagos]:
...
where c.id=p.idcliente and p.sede = '{sede}'
and date(p.fechapago) >=' {inicio} ' and date(p.fechapago)<='{fin}' and monto > 0 and f.id=p.idformadepago and t.id=p.idtaza   
            ...
            Logs.WirterTask(f"{self.ERROR} error en la base linea 168  de datos detail{e}")
            return ResponseInternal.responseInternal(False,"ERROR EN LA BASE DE DATOS",None)
        finally:
            Logs.WirterTask("Finalizada la ejecucion de registros de productos ")
            self.disconnect()
            
    def __countPunto__(self,sede: str,inicio:str,fin:str) -> list[PuntoCount]:
        ...
        try:
            ...
            if conexion['status'] == True:
              with self.conn.cursor() as cur :
                    ...
                    cur.execute(f"""
                           select coalesce(count(monto),0),coalesce(sum(monto * precio),0) from pagos_espacios as p 
inner join tazadollar t on precio=precio where idformadepago ={selector} AND sede ='{sede}' AND date(p.fechapago) >=' {inicio}' and date(p.fechapago)<='{fin}' and t.id = p.idtaza ;
    ...
    def __ventasPorProducto__(self,sede: str,inicio:str,fin:str) -> list[VentasPorProducto]:
        data=[]
        try:
            conexion=self.connect()
...
GROUP BY i.nombre;
                    ...
                    if count > 0 :
                        for i in cur :
                         ...
                         Logs.WirterTask(f"ventas por p´roducvtos VAlidacion {i}")
        ...
        try:
            ...
            if conexion['status'] == True:
              with self.conn.cursor() as cur :
                    ...
                    cur.execute(f"""
                           select idorden,c.nombre,p3.nombre ,p2.cantidad,p2.total,date(o.fechapedido) as fecha,to_char(o.fechapedido ,'hh12:MI:SS AM') as hora from pedidos_espacios p2 
left join productos_espacios  p3 on nombre=p3.nombre 
left join clientes c on c.nombre=c.nombre
left join ordenes_espacios o  on o.id =o.id
...
ORDER BY c.nombre ASC;
            ...
            Logs.WirterTask("Finalizada la ejecucion de registros de productos ")
            self.disconnect()
    def generarCierre(self,sede: str,inicio:str,fin:str):
        try:
            ventaCLientes=[]
            detallePago=[]
            ...
            html=''
            output_path =f"assets/reports/espacios/cierre/CierreByFechas{inicio+fin}{sede}{datetime.datetime.today()}.pdf"
            #print(type(self.__VentasClientes__(sede,inicio,fin)['response']))
            
            datos=ReporteEspaciosEntity(ventasPorProductos=self.__ventasPorProducto__(sede,inicio,fin)['response'],
                                        ...
                                        puntoCount=self.__countPunto__(sede,inicio,fin)['response'],
    ...
    def generarCierreHTML(self,sede: str,inicio:str,fin:str):
        try:
            ventaCLientes=[]
            detallePago=[]
            ventaCLientes=self.__VentasClientes__(sede,inicio,fin)['response']
            ...
            html=''
            #output_path =f"assets/reports/espacios/cierre/CierreDeJornada{sede}{datetime.datetime.today()}.pdf"
            
            
            datos=ReporteEspaciosEntity(ventasPorProductos=self.__ventasPorProducto__(sede,inicio,fin)['response'],
                                        deudaCliente=self.__DeudaCleinteBysede__(sede)['response'],
                                        detallePedidos=self.__DetallesPedidos__(sede,inicio,fin)['response'],
                                        walletDisponibles=self.__walletDisponible__(sede,inicio,fin)['response'],
                                        ventasPorCliente=ventaCLientes,
                                        ordenesAbiertasHistoricas=self.__OrdenesHistoricas__(sede)['response'],
                                        ordenesAbiertas=self.__OrdenesAbiertas__(sede,inicio,fin)['response'],
                                        detallePagos=detallePago,
            ...
            return ResponseInternal.responseInternal(True,"reporte de inventario generado con exito",html)
                ...
                Logs.WirterTask(f"finalizado el reporte de inventario de la sede {sede}")
    def integracion(self,sede,inicio,fin):
        datos=None
        try:
            ventaCLientes=[]
            detallePago=[]
            ...
            datos=ReporteEspaciosEntity(ventasPorProductos=ventasPorProductos,
                                        deudaCliente=self.__DeudaCleinteBysede__(sede)['response'],
                                        detallePedidos=self.__DetallesPedidos__(sede,inicio,fin)['response'],
                                        walletDisponibles=self.__walletDisponible__(sede)['response'],
                                        ventasPorCliente=ventaCLientes,
            ...
            return ResponseInternal.responseInternal(True,"integracion del core cierre espacios realizada con exito",datos)       
        except  Exception as e :
            traceback.print_exc()
            return ResponseInternal.responseInternal(False,f"integracion del core cierre espacios fallida detalles{e} ",datos)               
