...
inner join tazadollar t on precio=precio where idformadepago ={selector} AND sede ='{sede}' AND date(p.fechapago) =' {datetime.date.today()} ' and t.id = p.idtaza ;
...
where  p3.id = idproducto and c.id=o.idcliente  and o.id=p2.idorden  and date(o.fechapedido) ='{datetime.date.today()}' and o.sede='{sede}';      
...
ORDER BY c.nombre ASC;
    ...
    def generarCierre(self,sede: str):
        try:
            ...
            datos=ReporteEspaciosEntity(ventasPorProductos=self.__ventasPorProducto__(sede)['response'],
                                        deudaCliente=self.__DeudaCleinteBysede__(sede)['response'],
                                        detallePedidos=self.__DetallesPedidos__(sede)['response'],
                                        ...
                                        ordenesAbiertas=self.__OrdenesAbiertas__(sede)['response'],
                                        detallePagos=detallePago,
                                        puntoCount=self.__countPunto__(sede)['response'],
    ...
    def generarCierreHTML(self,sede: str):
        try:
            ventaCLientes=[]
            detallePago=[]
            ventaCLientes=self.__VentasClientes__(sede)['response']
    ...
    def integracion(self,sede):
        ...
        try:
            ...
            return ResponseInternal.responseInternal(False,"integracion del core cierre espacios fallida",datos)               
