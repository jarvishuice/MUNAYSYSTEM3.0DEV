                    ...
                    with self.conn.cursor() as cur:
                        cur.execute(f"""
                                   select id,nombre,apellido,ci,nombreusuario,password ,token ,status,tipouser,urlimg,ultima_sesion ,creado from usuarios
                                    ...
                                    """)
                        for i in cur:
                            data.append(UsersEntity(id=int(i[0]),nombre=i[1],apellido=i[2],ci=i[3],nombreusuario=i[4],password=i[5],token=i[6], status=i[7],tipoUsuario=int(i[8]),urlImagen=i[9],ultimaSesion=str(i[10]),creado=str(i[11]))) 
                        self.conn.commit()
