...
class PlantillaHTMLHistorialCliente():
    ...
    def __procesarPagos(self,datos:ClientReportEntity):
        ...
        if datos.pagos is not None:
            for i in datos.pagos:
                ...
                self.totalPagos+=float(i.monto)
                self.totalPagosBs+=bolos
    ...
    def getHtml(self,sede,datos:ClientReportEntity,fInicio,fFin):
        self.style = """<style>
        body {
    ...
    height: auto;
       ...
       <div class="parrafo"> <p >cliente:{datos.cliente.nombre}</br>
         ...
         <center> <h1> Historico de cliente en la sede {sede} del {fInicio} al {fFin}</h1> 
