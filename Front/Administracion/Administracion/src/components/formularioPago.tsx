  ...
  return ( <div className="formularioPago" key="FormularioPAgo">
    ...
    min: 0.1, // Valor mínimo
     // Valor máximo
      ...
      <Button  disabled={(Number(iMonto) < Number(totalApagar) || Number(age) < 1)} sx={{width:"30%"}} color="success" onClick={()=>{SaldarDeudas(0,abono,{
        id:'f',
        fecha:"f",
        monto:Number(iMonto),
        motivo:"pago",
      ...
      <Button color={"warning"}  disabled={((Number(iMonto)!==0 )&&(iMonto < Number(totalApagar) && Number(age) > 0))?false:true}  onClick={()=>{Multipago({
         id:"f",
         fecha:"f",
