...
export class WalletDAO extends(Iwallet){
    ...
    async RecargarWalletEspacios(pago:PagosEntity):Promise<PagosEntity|null>{
      try{ 
        console.log(pago)
        const response =await fetch(`${this.API}PagosWallet/RecargaWalletEspacios/`,{method:'POST',headers:this.headers,body:JSON.stringify(pago)});
        if (response.ok) {
            const data = await response.json();
            console.log(data)
            return data as PagosEntity;
