...
export function FormularioPagoEspacios(props: any): any {
  ...
  return (<div className="formularioPago" key="FormularioPAgo">
    ...
    <Box sx={{ mt: 2, p: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
      <Button disabled={(Number(iMonto) < Number(totalApagar)) ? true : false} sx={{ width: "30%" }} color="success" onClick={() => {
      ...
      <Button color={"warning"} disabled={Number(iMonto) != 0 && (iMonto < Number(totalApagar)) ? false : true} onClick={() => {
        Multipago({
          id: "f",
          fecha: "f",
