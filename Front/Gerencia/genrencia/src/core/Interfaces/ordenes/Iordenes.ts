...
export abstract class IOrdenes {
  /**
   ...
   /**
     ...
     * @returns A Promise that resolves to a boolean value indicating whether the deletion was successful or not.
     */
    abstract deleteOrden(idOrden:string,idUser:number,nombre:string,sede:string):Promise<boolean>;
    
    /**
     * Este servicio edita las ordenes abiertas solo los atributos de idcliente y fecha de apertura
     * @date 15/3/2024 - 9:55:21 a. m.
     *
     * @abstract
     * @param {OrdenesEntity} orden
     * @param {number} idUser 
     * @param {string} nombre 
     * @returns {Promise<boolean>}
     */
    abstract editOrden(orden:OrdenesEntity,idUser:number,nombre:string):Promise<boolean>;
